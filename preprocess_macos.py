...
def get_detector():
    ...
    if detector is None:
        ...
        print(f"[MTCNN] Inicializando detector no dispositivo: {device}")
...
def process_video(video_path: str, output_dir: str) -> int:
    ...
    frames_dir = Path(output_dir) / video_path.stem
      ...
      # 🧹 Remove arquivos ._ do macOS que bloqueiam criação de diretórios
    if frames_dir.exists() and frames_dir.is_file():
        print(f"[WARN] Removendo arquivo fantasma: {frames_dir}")
        frames_dir.unlink()
